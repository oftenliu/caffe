...
void OftenMtcnnLossLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  drop_loss_rate_ = this->layer_param_.mtcnn_loss_param().drop_loss_rate();
  if (drop_loss_rate_ < 0.f) drop_loss_rate_ = 0.f;
  if (drop_loss_rate_ > 1.f) drop_loss_rate_ = 1.f;
}

template<typename Dtype>
void OftenMtcnnLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template<typename Dtype>
void OftenMtcnnLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  int n1, n2, n3, n4;
  n1 = n2 = n3 = n4 = 0;
...
template<typename Dtype>
void OftenMtcnnLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(OftenMtcnnLossLayer)
#endif  // CPU_ONLY

INSTANTIATE_CLASS(OftenMtcnnLossLayer);
REGISTER_LAYER_CLASS(OftenMtcnnLoss);
